...
#include "SPIFFS.h"
#include <WiFiUdp.h>  // Normal UDP için
...
WiFiUDP udp2;  // Normal UDP için
const char* udpAddress = "192.168.137.22";
const int udpPort = 4210;

const char* udpAddress1 = "192.168.137.5";  // İkinci ESP32'nin IP'si
...
// Wakeword kontrolü için değişkenler
bool wakeword_detected = false;
const char* WAKE_WORD = "alexa";
const int WAKE_WORD_TIMEOUT = 5000; // 5 saniye timeout
unsigned long wakeword_start_time = 0;

// Fonksiyon prototipleri
String send_audio_chunk(uint8_t* data, size_t len, bool is_first, bool is_last, bool is_wakeword_check = false);
...
void send_sensor_data(const String& url, const String& data) {
  ...
  if (code == HTTP_CODE_OK) {
    Serial.println("✅ Sensör verisi sunucuya gönderildi");
  } else {
  ...
  // Diğer ESP32'ye UDP ile gönder
  udp2.beginPacket(udpAddress1, udpPort1);
  udp2.write((uint8_t*)data.c_str(), data.length());
  if (udp2.endPacket()) {
    Serial.println("✅ Sensör verisi diğer ESP32'ye gönderildi");
  } else {
    Serial.println("❌ UDP paketi gönderilemedi");
  }
}
...
String send_audio_chunk(uint8_t* data, size_t len, bool is_first, bool is_last, bool is_wakeword_check) {
  WiFiClient client;
  ...
  http.addHeader("X-Wakeword-Check", is_wakeword_check ? "true" : "false");
...
// Wakeword kontrolü için kısa ses kaydı
void check_wakeword() {
  Serial.println("\n🎙 Wakeword kontrolü yapılıyor...");
  Serial.println("(Lütfen 'Alexa' diyerek başlayın)");
  
  // Wakeword için ses kaydı başlat
  i2s_record_init();
  size_t bytes_read = 0;
  i2s_read(I2S_NUM_0, pcm_data, BUFFER_SIZE, &bytes_read, portMAX_DELAY);
  
  // Wakeword kontrolü için sesi gönder
  String wakeword_url = send_audio_chunk(pcm_data, bytes_read, true, true, true); // true = wakeword check
  if (wakeword_url.length() > 0) {
    // Wakeword kontrolü için yanıtı bekle
    waiting_for_response = true;
    play_wav_from_url(wakeword_url);
    wait_for_audio();
    waiting_for_response = false;
    
    // Wakeword yanıtını kontrol et
    if (wakeword_url == "wakeword_detected") {
      wakeword_detected = true;
      wakeword_start_time = millis();
      Serial.println("✅ Wakeword algılandı! Komut dinleniyor...");
    } else {
      Serial.println("❌ Wakeword algılanmadı. Tekrar deneyin...");
    }
  }
...
void setup() {
  ...
  esp_log_level_set("*", ESP_LOG_ERROR);  // Sadece hata loglarını göster
  esp_log_level_set("HTTPClient", ESP_LOG_ERROR);
  esp_log_level_set("WiFiClient", ESP_LOG_ERROR);
...
void loop() {
  ...
  // Wakeword timeout kontrolü
  if (millis() - wakeword_start_time > WAKE_WORD_TIMEOUT) {
    wakeword_detected = false;
    Serial.println("⏰ Wakeword zaman aşımı. Yeniden bekleniyor...");
    return;
  }
  ...
  for (int chunk = 0; chunk < TOTAL_TIME_SEC / CHUNK_TIME_SEC; chunk++) {
    ...
    url = send_audio_chunk(pcm_data, bytes_read, is_first, is_last, false); // false = normal komut
    if (url.length() && is_last) {
      ...
      wait_for_audio();
      waiting_for_response = false;
      wakeword_detected = false; // Wakeword'ü sıfırla
      break;
    }
